...
THEME = {
    "primary": "#04244A", "accent": "#00A9CE", "ink": "#1A1A1A",
    "muted": "#8A94A6", "hairline": "#E3E8EE", "bg": "#FFFFFF",
    "card": "#F4F6F8", "font": "'PT Sans', 'Helvetica Neue', Arial, sans-serif",
}
...
.subhead { background:%(accent)s; color:#fff; font-weight:700; font-size:15px;
           padding:7px 14px; border-radius:3px; display:inline-block; }
.body { font-size:15px; line-height:1.5; }
ul { list-style:none; }
.bul { font-size:15px; line-height:1.5; padding-left:20px; position:relative; margin:9px 0; }
.bul:before { content:"—"; color:%(accent)s; position:absolute; left:0; font-weight:700; }
...
.cover .veil { position:absolute; inset:0;
   background:linear-gradient(180deg, rgba(4,36,74,.35) 0%%, rgba(4,36,74,.55) 55%%, rgba(4,36,74,.92) 100%%); }
.cover .ct { position:absolute; left:60px; right:60px; bottom:70px; }
.cover .accentbar { width:90px; height:5px; background:%(accent)s; margin-bottom:22px; }
...
/* kpi card */
.kpi { background:%(primary)s; border-radius:8px; padding:26px 26px; color:#fff;
...
.kpi .lbl { font-size:11px; color:#9fb0c6; text-transform:uppercase; letter-spacing:.5px; }
...
.card { background:%(card)s; border-radius:8px; padding:22px 18px; border-top:5px solid %(accent)s; }
.card h4 { color:%(primary)s; font-size:17px; margin-bottom:12px; min-height:46px; }
...
.ev { background:%(primary)s; color:#fff; border-radius:8px; padding:18px 22px; margin-bottom:16px; }
.ev .lbl { color:%(accent)s; font-size:12px; text-transform:uppercase; letter-spacing:1px; }
.ev .val { font-size:30px; font-weight:800; margin-top:4px; }
...
def _slides(c, charts, images):
    ...
    # 2 exec summary
    sm = c["summary"]; kf = sm["key_figures"]
    ...
    cimg = f'<img class="imgband" src="{_img_uri(img("company"))}" style="margin-bottom:14px">' if img("company") else ""
    ...
    # 4 market
    mk = c["market"]
    risks = f'<div class="body" style="color:{THEME["muted"]};margin-top:14px;font-size:12px">Риски: {_e(_clip("; ".join(mk.get("risks") or []),200))}</div>' if mk.get("risks") else ""
    S.append(f'''<div class="slide"><div class="pad">
      <div class="kicker">{_e(_clip(c["company_name"],90))}</div>
    ...
    cards = "".join(f'<div class="card"><h4>{_e(_clip(it.get("title"),60))}</h4><p>{_e(_clip(it.get("text"),230))}</p></div>'
    ...
    # 7 valuation
    v = c["valuation"]
    method_just = (("Метод: " + _clip(v.get("method"), 90) + ". ") if v.get("method") else "") + _clip(v.get("justification", ""), 300)
    S.append(f'''<div class="slide"><div class="pad">
      <div class="kicker">{_e(_clip(c["company_name"],90))}</div>
...
def render(content: dict, charts: dict, images: dict, out_pdf="teaser.pdf") -> str:
